...
def go(arg):
    ...
    # create the model
    model = GenerateTransformer.GenerateTransformer(k=arg.embedding_size, heads=arg.num_heads, depth=arg.depth, seq_length=arg.context, num_tokens=NUM_TOKENS, wide=arg.wide)
            ...
            with torch.no_grad():
                ...
                for current in range(data_sub.size(0)):
                    ...
                    if len(batch) == arg.test_batchsize or current == data_sub.size(0) - 1:
                        ...
                        lnprobs = output[torch.arange(b, device='cuda', -1, target]
